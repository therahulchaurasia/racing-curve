...
- **`SettingsCog.tsx`** — pixel gear graphic (two crossed pluses, masked centre hole) as a self-contained bordered icon button. Fixed top-right; click or `Escape` opens the modal.
- **`SettingsDialog.tsx`** — radix `Dialog` (focus trap, ESC/backdrop close, ARIA — we only skin it). DOOM-style: `RacingCurvesLogo` header above the panel (asphalt fill, white text), centered `SETTINGS` title, near-black overlay with a soft circular white vignette, top-anchored (`top: 7vh`, grows downward), single centered crimson `CLOSE` (no X). Exports the shared **`SettingRow`** (label + control) and **`SettingSegmented`** (N-option segmented toggle, active = violet/bright, rest dim) — use these for every settings row, don't reinvent per-row controls.
