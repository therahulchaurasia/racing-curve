...
// Two smooth mountain ridges for the night backdrop. Each silhouette is the UPPER ENVELOPE (smooth-
// max) of rounded cosine "bumps" placed at varied x positions, heights and widths — overlapping
// bumps merge into a connected range with distinct peaks, and the gaps between read as valleys.
// (Additive noise made soft dunes; a sine made identical hills. Envelope-of-bumps reads as actual
// mountains.) The BACK ridge is the darker, TALLER range (its peaks rise above the front silhouette,
// so it's the dominant far mass); the FRONT ridge is lighter + shorter, sitting in front of it.
//
// preserveAspectRatio="none" stretches a fixed bump count to the container, so the same peaks get
// wider + taller on bigger screens (count stays constant). Draft in /process; lock, then port.
...
// sampling step (small = smooth curve).
const VB_W = 1500
const VB_H = 280
const SAMPLE = 3
...
export type Ridge = {
  ...
  peaks: number // number of bumps placed across the width (front only; back derives from front)
  seed: number
}
...
// stays high). Each ridge (back/front) is generated independently from its own seed + peak count.
function layoutBumps(seed: number, peaks: number): Bump[] {
  const rng = mulberry32(seed)
...
function envPoints(bumps: Bump[], base: number, amp: number, groundSeed: number): string {
